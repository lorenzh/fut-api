...
module.exports = function(options){
  ...
  login.prototype.login = function(email, password, secret, tfCodeCb, loginCb){
    ...
    if(!secret || !__.isString(secret) || email.trim().secret <= 0)
  ...
  login.prototype.existinglogin = function(loginCb){
    defaultRequest = request.defaults({
      jar: jar,
      followAllRedirects: true,
      gzip: true,
  ...
  function phishing() {
    console.log(loginResponse.sessionData.sid);
    defaultRequest = defaultRequest.defaults({
        headers: {
