...
module.exports = function(options){
  ...
  if(options) defaultOptions = utils.mergeOptions(defaultOptions,options);
  ...
  var login = function(){
    var that = this;
    if(defaultOptions.cookieJson) that.setCookieJarJSON(defaultOptions.cookieJson);
  };

  login.prototype.options = defaultOptions;

  login.prototype.login = function(email, password, secret, tfCodeCb, loginCb){
    if(!email || utils.isStringNullOrWhitespace(email))
      return loginCb(new Error("Email is empty."));

    if(!password || utils.isStringNullOrWhitespace(password))
      return loginCb(new Error("Password is empty."));

    if(!secret || utils.isStringNullOrWhitespace(secret))
      return loginCb(new Error("Secret is empty."));

    if(!utils.isFunction(tfCodeCb))
      return loginCb(new Error("tfCodeCb is not a function."));

    if(!utils.isFunction(loginCb))
      return loginCb(new Error("tfCodeCb is not a function."));

    loginDetails = { "email": email, "password": password, "secret": hasher(secret), "tfCodeCb": tfCodeCb, "loginCb": loginCb };
  ...
  function getAccount() {
    loginResponse.shard = loginResponse.shardInfos.shift();
    ...
    defaultRequest.get(urls.login.accounts, function(error, response, body) {
      ...
      if(!body || body.code === "465")
        if(loginResponse.shardInfos.length > 0)
            return getAccount();
  ...
  function getSession(){
    var gameSku = Object.keys(loginResponse.persona.userClubList[0].skuAccessList)[0];
    ...
    var data = {
        ...
        "gameSku": gameSku,
        "nucleusPersonaPlatform": loginResponse.persona.userClubList[0].platform,
        "locale": "en-GB",
        "method": "authcode",
        "priorityLevel": 4,
